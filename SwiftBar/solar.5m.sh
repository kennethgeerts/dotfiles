...
api = GoodWeApi("X", "X", "X")
payload = { 'powerStationId' : api.system_id }
data = api.call("v2/PowerStation/GetMonitorDetailByPowerstationId", payload)

pv = data['inverter'][0]['out_pac']
battery = data['inverter'][0]['invert_full']['soc']
load = data['powerflow']['load'].split('(')[0].split('.')[0]
day = data['inverter'][0]['eday']
month = data['inverter'][0]['emonth']
total = data['inverter'][0]['etotal']
print(":sun.max:%.0fW :bolt:%sW :bolt.fill.batteryblock:%.0f%% | size = 11" % (pv, load, battery))
print("---")
print("Day: %.2fkWh" % day)
print("Month: %.2fkWh" % month)
